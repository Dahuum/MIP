...
python3 -m uvicorn api_server:app --host 0.0.0.0 --port 5000 > .pids/api.log 2>&1 &
API_PID=$!
...
echo -e "${BLUE}⚛️  Building and starting Dashboard...${NC}"

# Check if .next exists (production build)
if [[ -d ".next" ]]; then
    echo -e "${CYAN}   Using existing build...${NC}"
else
    echo -e "${CYAN}   Building Next.js app (this may take a moment)...${NC}"
    npm run build --silent 2>/dev/null
fi

# Start the production server
npm run start > .pids/dashboard.log 2>&1 &
DASHBOARD_PID=$!
